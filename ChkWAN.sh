...
if [ "$1" == "status" ];then                                            # v1.15
        if [ -n "$(ps -w | grep $(basename $0) | grep -v "grep $(basename $0)" | grep -v "status")" ];then
                ...
                if [ -f $LOCKFILE ];then                                        # v1.05
                        Say "$VER $TS $COUNT $FAILTIMESCOUNT Check WAN monitor ACTIVE" $(grep -oE "PID=[0-9]*" $LOCKFILE)
                else
                        echo -e $cBYEL"\n"
                        Say "$VER $TS $COUNT $FAILTIMESCOUNT Check WAN monitor termination request pending....."           # v1.05
                fi
                echo -e $cRESET
                exit
        else
                echo -e $cBMAG"\n"
                Say "$VER $TS $COUNT $FAILTIMESCOUNT Check WAN monitor not running"
                echo -e $cRESET
                exit
                fi
fi
...
if [ "$1" == "stop" ];then
        if [ -f $LOCKFILE ];then                                                # v1.05
                echo -e $cGRE"\n"
                Say "$VER $TS $COUNT $FAILTIMESCOUNT Check WAN monitor Termination requested" $(grep -oE "PID=[0-9]*" $LOCKFILE)
...
if [ "$uptime_seconds" -lt 600 ]; then
  Say  $VER $TS $COUNT $FAILTIMESCOUNT "ChkWAN Skipped run: uptime" ${uptime_seconds}"s (< 600s threshold)"
  exit 0
fi
...
if [ "$(echo $@ | grep -cw 'nowait')" -eq 0 ] && [ "$QUIET" != "quiet" ];then
        echo -e $cBCYA
        Say  $VER $TS $COUNT $FAILTIMESCOUNT $WAN_NAME "connection status monitoring will begin in" $INTERVAL_ALL_FAILED_SECS "seconds....."
...
if [ "$QUIET" != "quiet" ];then
        echo -en $cBWHT
        Say "$VER $TS $COUNT $FAILTIMESCOUNT Monitoring pass" $(($FAIL_CNT+1)) "out of" $TRIES
fi
...
while [ $FAIL_CNT -lt $MAX_FAIL_CNT ]; do
        ...
        FAIL_CNT=0
                ...
                if [ -z "$(cru l | grep "$SNAME")" ];then
                        if [ -z "$ONCE" ];then
                                if [ "$QUIET" != "quiet" ];then
                                        Say "$VER $TS $COUNT $FAILTIMESCOUNT Monitoring" $WAN_NAME $WAN_INDEX  $DEV_TXT "connection OK.....("$TXT"). Will check" $WAN_NAME "again in" $INTERVAL_SECS "secs"
                                        echo -en $cRESET
                                fi
                                ...
                                Say "$VER $TS $COUNT $FAILTIMESCOUNT Monitoring" $WAN_NAME $WAN_INDEX  $DEV_TXT "connection OK.....("$TXT")."
                                echo -en $cRESET
                                flock -u $FD                                                    # v1.15
                                exit 0
                        fi
...
if [ "$ACTION" == "WANONLY" ];then
        Say "$VER $TS $COUNT $FAILTIMESCOUNT Renewing DHCP and restarting" $WAN_NAME $WAN_INDEX "(Action="$ACTION")"
        killall -USR1 udhcpc
        sleep 10
        if [ -z "$WAN_INDEX" ];then
                service restart_wan
        ...
        Say "$VER $TS $COUNT $FAILTIMESCOUNT Rebooting..... (Action="$ACTION")"
        echo -e "\n\t\t**********Rebooting**********\n\n"$cBGRE
